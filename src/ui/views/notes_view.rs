...
pub struct NotesView {
    ...
    library_group_filters: Vec<i64>,
    tag_filter: Entity<ComboboxState<SearchableVec<GroupChoice>>>,
...
impl NotesView {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        cx.subscribe(&tag_filter, |this, _, event, cx| {
            if let ComboboxEvent::Change(group_ids) = event {
                this.library_group_filters = group_ids.clone();
                this.notes_scroll.set_offset(point(px(0.), px(0.)));
                cx.notify();
            }
        })
        .detach();
                ...
                async move {
                    ...
                    if let Ok(groups) = result {
                        ...
                        let filter_state = this.update(&mut cx, |this, cx| {
                            this.groups = groups;
                            this.library_group_filters
                                .retain(|id| this.groups.iter().any(|group| group.id == *id));
                            ...
                            cx.notify();
                            (
                                this.tag_filter.clone(),
                                this.library_group_filters.clone(),
                                this.window,
                            )
                        });
                        if let Ok((tag_filter, selected, window_handle)) = filter_state {
                            cx.update_window(window_handle, move |_, window, cx| {
                                tag_filter.update(cx, |state, cx| {
    ...
    fn show_reader(&mut self, id: i64, cx: &mut Context<Self>) {
        self.page = NotesPage::Reader(id);
        self.detail_scroll.set_offset(point(px(0.), px(0.)));
        cx.notify();
    }
                ...
                .child(
                    v_flex()
                        ...
                        .child(div().text_sm().child(
                            "选择已有分组，或输入新名称后保存。知识蓝图和复习进度不会改变。",
                        ))
                    ...
                    v_flex()
                        ...
                        .child(
                            div()
                                ...
                                .child(
                                    "直接粘贴原始网页、课程笔记或多篇 Markdown。AI 会自动去除导航与广告，拆分材料，生成标题并建立知识蓝图。",
                                ),
                        )
                        .child(
                            v_flex()
    ...
    fn confirm_delete_note(
        ...
        window.open_alert_dialog(cx, move |dialog, _, _| {
            dialog
                .title("删除这篇笔记？")
                .description(format!(
                    "“{title}”以及它的知识蓝图、动态题面和复习记录都会被删除，此操作无法撤销。"
                ))
                .button_props(
                    DialogButtonProps::default()
                        .ok_text("删除")
...
impl Render for NotesView {
    fn render(&mut self, window: &mut gpui::Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let header = match self.page {
                    ...
                    h_flex()
                        ...
                        .child(
                            Button::new("rename-group")
                                ...
                                .when(!compact, |this| this.label("重命名分组"))
                                .outline()
                                .tooltip("重命名分组")
            ...
            NotesPage::Detail(note_id) => {
                ...
                page_header(
                    RuizIcon::NotebookText,
                    title,
                    "查看知识蓝图、复习准备状态和整理后的完整原文。",
                    Some(
                        Button::new("back-to-library")
                            .icon(IconName::ArrowLeft)
                            .when(!compact, |this| this.label("返回资料库"))
                            .outline()
                            .tooltip("返回资料库")
            ...
            NotesPage::Reader(note_id) => {
                let title = self
                    .notes
                    .iter()
                    .find(|note| note.id == note_id)
                    .map(|note| note.title.clone())
                    .unwrap_or_else(|| "完整原文".into());
                page_header(
                    IconName::BookOpen,
                    title,
                    "浏览 AI 去除网页外壳和重复目录后保留的完整正文。",
                    Some(
                        Button::new("back-to-note-detail")
                            .icon(IconName::ArrowLeft)
                            .when(!compact, |this| this.label("返回笔记详情"))
                            .outline()
                            .tooltip("返回笔记详情")
                            .on_click(cx.listener(move |this, _, _, cx| {
                                this.show_detail(note_id, cx);
                            }))
                            .into_any_element(),
                    ),
                    cx,
        ...
        let visible_note_count = self
            .notes
            .iter()
            .filter(|note| matches_group_filter(note.group_id, &library_filters))
            .count();
        let library_title = match library_filters.as_slice() {
            [] => "全部材料".to_string(),
            [group_id] => self
                .groups
                .iter()
                ...
                .map(|group| group.name.clone())
                .unwrap_or_else(|| "全部材料".into()),
            group_ids => format!("已选 {} 个标签", group_ids.len()),
        };

        // 一级资料库：标签筛选与材料列表使用同一条响应式内容轨道。
        let notes_list = div()
            .id("notes-list-wrap")
            .relative()
            .flex_1()
            ...
            .child(
                v_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            .child(
                                v_flex()
                                    .gap_0p5()
                                    .child(div().font_semibold().child(library_title))
                                    .child(
                                        div()
                                            .text_sm()
                            ...
                            .child(
                                div()
                                    ...
                                    .text_sm()
                                    .text_color(colors.muted_foreground)
                                    .child(format!("{visible_note_count} 篇")),
                            ),
                    )
                    .child(
                        v_flex()
                    ...
                    .when(!self.notes_loading && visible_note_count == 0, |this| {
                        this.child(
                            v_flex()
                                .flex_1()
                                .items_center()
                    ...
                    .children(
                        (!self.notes_loading)
                            ...
                            .into_iter()
                            .flat_map(|_| {
                                let active_filters = library_filters.clone();
                                self.notes
                                    .iter()
                                    .filter(move |note| {
                                        matches_group_filter(note.group_id, &active_filters)
                                    })
                                    .map(|note| {
                                        let id = note.id;
                                        let title = note.title.clone();
        ...
        // 原文阅读使用独立页面，避免在知识蓝图中塞入正文预览。
        let detail =
            if let NotesPage::Reader(id) = self.page {
                ...
                v_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            .justify_between()
                            .gap_3()
                            .child(div().text_sm().font_medium().child("完整原文"))
                            .child(div().text_xs().text_color(colors.muted_foreground).child(
                                format!(
                                    "{} 个字符",
                                    note.map(|note| note.content.chars().count()).unwrap_or(0)
                                ),
                            )),
                    )
                    .child(
                    ...
                    .into_any_element()
            } else if let Some(id) = self.page.note_id() {
...
fn matches_group_filter(group_id: i64, selected_group_ids: &[i64]) -> bool {
...
mod tests {
    ...
    fn empty_group_filter_includes_every_material() {
        assert!(matches_group_filter(42, &[]));
    }
